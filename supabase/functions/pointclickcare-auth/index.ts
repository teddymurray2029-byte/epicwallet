...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!credentialsConfigured) {
      return new Response(
        JSON.stringify({ error: 'PointClickCare OAuth credentials not configured' }),
    ...
    const pccFhirBase = 'https://fhir.pointclickcare.com/fhir';
    ...
    if (action === 'authorize') {
      ...
      if (upsertError) {
        const { data: existing } = await supabase
          ...
          .eq('integration_type', 'pointclickcare')
          .maybeSingle();

        if (existing) {
          await supabase
            .from('ehr_integrations')
            .update({ auth_state: stateToken, client_id: pccClientId })
            .eq('id', existing.id);
        } else {
      ...
      authUrl.searchParams.set('client_id', pccClientId);
      authUrl.searchParams.set('redirect_uri', redirectUri);
      authUrl.searchParams.set('state', stateToken);
      authUrl.searchParams.set('scope', 'openid fhirUser launch/patient');
    ...
    if (code && state) {
      ...
      try {
        const subscriptionRes = await fetch(`${pccFhirBase}/Subscription`, {
          method: 'POST',
          headers: {
