...
export default function InviteAccept() {
  ...
  useEffect(() => {
    const loadInvite = async () => {
      ...
      if (fetchError) {
        ...
        setError('Unable to load this invite. Please try again later.');
        setLoading(false);
        return;
      }

      if (!data) {
        setError('This invite link is invalid or has already been removed.');
        setLoading(false);
        return;
      }
  ...
  const handleAcceptInvite = async () => {
    ...
    if (!entity) {
      setError('Your wallet needs to be registered before accepting this invite.');
      return;
    }
    ...
    if (entity.organization_id === invite.organization_id) {
      toast({
        ...
        description: 'Your wallet is already linked to this organization.',
      });
      return;
    }
    ...
    if (entityError) {
      console.error('Failed to update entity organization', entityError);
      setError('We could not add you to the organization. Please try again.');
    ...
    if (inviteError) {
      ...
      setError('Invite accepted, but we could not finalize the invite status.');
      setIsAccepting(false);
      return;
    }
    ...
    toast({
      ...
      description: 'Your wallet is now linked to the organization.',
    });
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <Card className="w-full max-w-lg">
          ...
          <CardContent className="space-y-4">
            ...
            {!loading && invite && !error && (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  You&apos;re about to join organization <span className="font-medium">{invite.organization_id}</span>.
                </p>
                {invite.expires_at && (
                ...
                {invite.used_at && (
                  <Alert>
                    ...
                    <AlertDescription>This invite link has already been used.</AlertDescription>
                  </Alert>
                )}
                {isExpired && (
                  <Alert>
            ...
            {isConnected && entityLoading && (
              <p className="text-sm text-muted-foreground">Loading your wallet profile...</p>
            )}
            ...
            {isConnected && !entityLoading && !entity && (
              <Alert>
                ...
                <AlertDescription>
                  Register your wallet first, then return to this invite link to join the organization.
                </AlertDescription>
              </Alert>
            )}
