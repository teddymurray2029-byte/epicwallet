...
export default function Organizations() {
  ...
  const handleGenerateInvite = async () => {
    ...
    const { error: inviteError } = await (supabase
      ...
      .insert({ organization_id: organizationId, token, created_by: entity.id, expires_at: expiresAt }) as any);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="animate-fade-in-up">
          <h1 className="text-2xl font-bold tracking-tight text-gradient-hero inline-block">Organization Management</h1>
          ...
          <Card className="animate-fade-in-up" style={{ animationDelay: '160ms' }}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center">
                <Button onClick={handleGenerateInvite} disabled={isGenerating || !canGenerateInvite} className="transition-all duration-200 hover:shadow-[var(--shadow-glow-teal)]">
                  <Link2 className="mr-2 h-4 w-4" />
                  {isGenerating ? 'Generating invite...' : 'Generate invite link'}
                </Button>
                {inviteExpiry && (
                  <span className="text-xs text-muted-foreground">Expires on {new Date(inviteExpiry).toLocaleDateString()}.</span>
                )}
              </div>
          ...
          <Card className="animate-fade-in-up" style={{ animationDelay: '240ms' }}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                {!isOrganizationOwner && (
                  <span className="text-xs text-muted-foreground">Only the organization owner can update Epic API details.</span>
                )}
                <Button onClick={handleSaveEpic} disabled={!isOrganizationOwner || isSavingEpic} className="sm:ml-auto">
